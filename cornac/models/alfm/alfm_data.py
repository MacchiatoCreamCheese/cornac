...
def jsd_similarity(theta_u_a, theta_v_a, max_val=1e7):
    """s = 1 - JSD(theta_u_a, theta_v_a), faithful to getTopicPartFactor.JSDist.

    Base-2 log; the exact edge cases of the Java ``KLDis``: terms where u_k==0 are
    skipped, a v_k==0 (with u_k>0) contributes ``max_val``, otherwise
    ``u_k * log2(u_k / (0.5(u_k+v_k)))``. ``JSDist = 1 - 0.5*(KL(u,v)+KL(v,u))``.
    """
    ...
    js = 0.5 * (_kl(theta_u_a, theta_v_a) + _kl(theta_v_a, theta_u_a))
    return 1.0 - js


def compute_topic_part(u_idx, i_idx, pi, lambda_u, lambda_v, theta_u, theta_v):
    """topicPart[a] = (pi*lambdaU[a] + (1-pi)*lambdaV[a]) * (1 - JSD(thetaU[a], thetaV[a])).

    Vectorized JSD across aspects. ``theta_u``/``theta_v`` are (A, K) arrays for the
    given user/item; ``lambda_u``/``lambda_v`` are (A,) arrays; ``pi`` a scalar.
    """
    A = theta_u.shape[0]
    tp = np.empty(A, dtype=np.float64)
    p = pi
    for a in range(A):
        ratio = p * lambda_u[a] + (1.0 - p) * lambda_v[a]
        s = jsd_similarity(theta_u[a], theta_v[a])
        tp[a] = ratio * s
    return tp
