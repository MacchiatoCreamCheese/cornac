...
def run_cd(base_model, order, defaults, eval_method, cache_path, verbose=True):
    ...
    def evaluate(params):
        ...
        if k in cache:
            if verbose:
                print(f"    [cache] {params} -> RMSE={cache[k]:.4f}")
            return cache[k], True
        t0 = time.time()
        ...
        if verbose:
            print(f"    [fit {secs:6.1f}s] {params} -> RMSE={score:.4f}")
        del model
        return score, False
    ...
    for param, values in order:
        if verbose:
            print(f"  Sweeping '{param}' over {values} (others locked)")
        ...
        if verbose:
            print(f"  -> locked {param}={best_val} (RMSE={best_score:.4f})")
